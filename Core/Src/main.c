...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  /* USER CODE BEGIN Callback 1 */
	...
	if(contaPing >= DT_PING){
		contaPing = 0;
		if(pingOK) //Checa se esta recebendo oper!
		    {
		     pingOK = 0;     //Recebeu oper!, manda oper! de volta
		     STR_BUFF(PNGPRG);
		     oQueEnv = sndPING; }
		    else {
		         tempoSemResposta++; //Não recebeu oper!, se não recebeu oper! 3x entra em estado de erro
		         if(tempoSemResposta >= 3)
		        {
		        	estado = 7;}
		    }
		}
